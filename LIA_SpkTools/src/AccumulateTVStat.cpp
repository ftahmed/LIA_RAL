...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
void TVAcc::loadEV(const String& evFilename, Config& config){	///load an EigenVoices Matrix
	...
	cout << "(AccumulateTVStat) Init EV matrix from "<< filename <<"  for EigenVoices Matrix: "<<", rank: ["<<_V.rows() << "] sv size: [" << _V.cols() <<"]"<<endl;
}
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------
void TVAcc::initEV(Config& config){		///random initialisation of the EigenVoices Matrix
		_rankEV=config.getParam("eigenVoiceNumber").toULong();
		_V.setDimensions(_rankEV,_svSize);
...
try{
		...
		if (verboseLevel >=1) cout << "(AccumulateTVStat) Random Init for EigenVoices Matrix with "<<randomLaw<<" law "<<", rank: ["<<_V.rows() << "] sv size: [" << _V.cols() <<"]"<<endl;
}
catch (Exception& e){ 
...
void TVAcc::estimateAndInverseLUnThreaded_EV(Config& config){
	...
	if (verboseLevel >= 1) cout << "(AccumulateTVStat) Compute and Inverse L Matrix for EigenVoices "<<endl;
...
void TVAcc::estimateAndInverseLThreaded_EV(unsigned long NUM_THREADS,Config& config){
	...
	if (verboseLevel >= 1) cout << "(AccumulateTVStat) Compute and Inverse L Matrix for EigenVoices Threaded"<<endl;
	if (NUM_THREADS==0) throw Exception("Num threads can not be 0",__FILE__,__LINE__);
